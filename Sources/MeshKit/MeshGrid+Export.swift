...
extension MeshColorGrid {
    ...
    public func export(to url: URL = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString + ".png"),
                       size: MeshPoint = .init(x: 1920, y: 1080),
                       pixelFormat: MTLPixelFormat = .bgra8Unorm,
                       colorSpace: CGColorSpace? = .init(name: CGColorSpace.linearSRGB)) async throws -> URL {
        return try await withCheckedThrowingContinuation({ continuation in
            let grid = self.asControlPoint()
            let dataProvider = MeshGradientState.static(grid: grid)
                .createDataProvider()
            let renderer = MetalMeshRenderer(metalKitView: nil, meshDataProvider: dataProvider, viewportSize: .init(x: size.x, y: size.y), grainAlpha: MeshGradientDefaults.grainAlpha)
            ...
            metalLayer?.frame = CGRect(origin: .zero, size: CGSize(width: CGFloat(size.x),
...
extension MTLTexture {
    ...
    func makeImage(colorSpace: CGColorSpace? = nil) -> CGImage? {
        ...
        guard let colorSpace = CGColorSpace(name: colorSpace?.name ?? CGColorSpace.linearSRGB) else { return nil }
